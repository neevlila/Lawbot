...
const ChatbotPageContent: React.FC = () => {
  const { toggleSideNav, isSideNavOpen, startNewSession, isBotTyping } = useChat();
  ...
  // Lightweight debug log so we can diagnose blank-screen issues quickly
  useEffect(() => {
    if ((import.meta as any).env?.DEV) console.log('ChatbotPageContent mounted — isBotTyping:', isBotTyping);
  }, [isBotTyping]);

  return (
    <div className="h-screen flex flex-col bg-light-bg dark:bg-dark-bg transition-colors duration-700 ease-in-out overflow-hidden">
      <BackgroundAnimation />
